...
Nguồn thu của platform là phí subscription: chủ quán (`owner`) và nhà cung cấp (`provider`)
mua gói theo tháng/năm qua [payOS](https://payos.vn).
...
3. payOS gọi `POST /api/payments/webhook` (đã verify chữ ký) → hệ thống kích hoạt subscription (đang có gói active thì cộng nối tiếp từ ngày hết hạn).
